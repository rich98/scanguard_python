...
def check_privileges():
    """
    Ensures the program is running with the appropriate privileges:
    - Root on Unix/Linux
    - Administrator on Windows
    """
    if os.name != 'nt':
        try:
            # On Unix-like systems, os.geteuid() returns the effective user ID
            # A value of 0 indicates the root user
            if os.geteuid() != 0:
                # If not running as root, terminate the program with an informative message
                sys.exit("❌ Scan Guard Dog requires elevated privileges.\n🔐 Please run it as root (e.g., 'sudo python3 app.py').")
        except AttributeError:
            # If os.geteuid() is not available, it's unclear whether the user has sufficient privileges
            logging.warning("Could not determine effective UID. Root access may be required.")
            sys.exit("❌ Could not verify user privileges. Root access may be required.")
    else:
        try:
            # ctypes.windll.shell32.IsUserAnAdmin() checks for administrator privileges
            if not ctypes.windll.shell32.IsUserAnAdmin():
                # If not running with admin rights, terminate with a prompt to run as Administrator
                sys.exit("❌ Scan Guard Dog requires Administrator rights on Windows.\n🔐 Please right-click and 'Run as Administrator'.")
        except Exception:
            # If an error occurs while checking admin status, exit with a generic error message
            sys.exit("❌ Unable to verify Administrator rights on Windows.")
...
class NetworkMonitor:
    ...
    def __init__(self, iface=None, port_threshold=1, time_window=10, icmp_threshold=2, arp_threshold=5, udp_threshold=3, arp_enabled=True, icmp_enabled=True):
    ...
    def detect_scan(self, pkt):
        ...
        if pkt.haslayer(IP):
            ...
            # print(f"[DEBUG] ARP packet seen. Enabled? {self.arp_enabled}")  # <-- ADD THIS LINE
            if self.arp_enabled:
                self._check_arp(pkt)
