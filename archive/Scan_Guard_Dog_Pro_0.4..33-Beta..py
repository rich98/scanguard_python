...
from logging.handlers import RotatingFileHandler, SysLogHandler  # Log handler that automatically rotates log files; SysLogHandler for syslog support
...
class NetworkMonitor:
    ...
    def __init__(self, iface=None, port_threshold=1, time_window=10, icmp_threshold=1, arp_threshold=1, udp_threshold=3, arp_enabled=True, icmp_enabled=True):
        ...
        # ================= Syslog Setup START ====================
        self.syslog_enabled = False  # Default disabled

        try:
            self.syslog_logger = logging.getLogger("SyslogLogger")
            self.syslog_logger.setLevel(logging.INFO)

            # Define your syslog server IP and port here
            syslog_address = ("192.168.0.172", 514)  # <-- Replace with your actual syslog server

            syslog_handler = SysLogHandler(address=syslog_address)
            syslog_formatter = logging.Formatter('%(asctime)s ScanGuardDog: %(message)s', datefmt='%b %d %H:%M:%S')
            syslog_handler.setFormatter(syslog_formatter)

            self.syslog_logger.addHandler(syslog_handler)
        except Exception as e:
            logging.warning(f"Syslog initialization failed: {e}")
        # ================= Syslog Setup END ======================
    ...
    def log_abnormality(self, key, detail, level="warning"):
        ...
        if self.syslog_enabled:
            try:
                ...
                logging.warning(f"Failed to send alert to syslog: {e}")
    ...
    def set_syslog_enabled(self, enabled):
        self.syslog_enabled = enabled
        logging.info(f"Syslog logging {'enabled' if enabled else 'disabled'}.")
    ...
    def detect_scan(self, pkt):
        ...
        # SECURITY: Drop malformed or irrelevant packets early to reduce attack surface
        try:
            if not (pkt.haslayer(IP) or pkt.haslayer(ARP) or pkt.haslayer(IPv6)):
                return  # Ignore non-IP/ARP/IPv6 packets
        except Exception as e:
            logging.warning(f"Malformed packet skipped: {e}")
            return

        # Flood detection (silent alert or countermeasure)
        if hasattr(self, "gui_callback_ref") and callable(self.gui_callback_ref):
            app = self.gui_callback_ref()
            if app.flood_protect.get() and len(self.packet_timestamps) > 500:
                message = "⚠️ Flood protection active — packet dropped silently to preserve GUI responsiveness."
                app.flood_dropped_count += 1  # Track dropped packets for stats
                logging.info(message)
                # Optional: show in GUI console with neutral or warning color
                if hasattr(app, "log_display"):
                    try:
                        app.log_display.configure(state='normal')
                        app.log_display.insert(tk.END, message + "\n", "info")  # Or "warning"
                        app.log_display.configure(state='disabled')
                        app.log_display.yview(tk.END)
                    except Exception:
                        pass
                return

        # Still log flood if user wants visibility
        if len(self.packet_timestamps) > 500:
            logging.warning("High packet rate detected (>500/sec) — possible flood condition.")
            # You may add flood countermeasures here if needed
            # return  # Uncomment to drop packet under flood conditions
            # Could also add GUI alert or throttle detection temporarily

        # Track active source IPs for host visibility
        if pkt.haslayer(IP):
        ...
        self.packet_count += 1  # Increment total packet count
        ...
        try:
            if pkt.haslayer(IP):
                ...
                self._check_udp(pkt)
            elif pkt.haslayer(ARP) and self.arp_enabled:
                self._check_arp(pkt)
            elif pkt.haslayer(IPv6) and self.icmp_enabled:
                self._check_icmpv6(pkt)
        except Exception as e:
            logging.error(f"Unhandled exception in detect_scan: {e}")
...
class NetworkMonitorApp:
    ...
    def __init__(self, root):
        ...
        self.syslog_enabled = tk.BooleanVar(value=False)
        self.flood_protect = tk.BooleanVar(value=True)
        ...
        ToggleSwitch(toggle_frame, "Syslog", self.syslog_enabled, command=self.toggle_syslog).grid(row=0, column=3, padx=20)
        ToggleSwitch(toggle_frame, "Flood Protect", self.flood_protect, command=self.toggle_flood_protect).grid(row=0, column=5, padx=20)
       ...
       # Top row buttons      
        ...
        self.log_display.tag_config("flood", foreground="#FFD700")  # Bright gold
    ...
    def toggle_flood_protect(self):
        state = self.flood_protect.get()
        msg_gui = "✅ Flood Protection ENABLED — high-rate packets will be silently dropped." \
            if state else "⚠️ Flood Protection DISABLED — all packets will be analyzed."
        
        msg_log = "Flood Protection ENABLED — high-rate packets will be silently dropped." \
            if state else "Flood Protection DISABLED — all packets will be analyzed."

        # Log it visually in GUI
        self.log_display.configure(state='normal')
        self.log_display.insert(tk.END, msg_gui + '\n', "flood")
        self.log_display.configure(state='disabled')
        self.log_display.yview(tk.END)

        # Log safe ASCII-only version
        logging.info(msg_log)
    
    
    def toggle_syslog(self):
        if self.monitor:
            self.monitor.set_syslog_enabled(self.syslog_enabled.get())
    ...
    # Create NetworkMonitor with GUI toggle options
        self.monitor = NetworkMonitor(
            iface=iface,
            arp_enabled=self.arp_enabled.get(),
            icmp_enabled=self.icmp_enabled.get()
        )
        self.monitor.gui_callback_ref = lambda: self  # Allow monitor to access GUI state
        self.toggle_flood_protect()  # Log initial state to GUI and file

        if self.flood_protect.get():
            msg = "✅ Flood Protection is ENABLED — packets over threshold will be dropped silently."
        else:
            msg = "⚠️ Flood Protection is DISABLED — all packets will be analyzed, even during high traffic."
        ...
        self.log_display.yview(tk.END)
            ...
            # Apply syslog toggle from GUI
        self.monitor.set_syslog_enabled(self.syslog_enabled.get())  # ← Enable syslog if checked

        # Hook GUI callback for real-time alerts
        self.monitor.gui_callback = self.add_abnormality_to_grid

        self.flood_dropped_count = 0  # Reset counter when monitoring starts
    ...
    def update_stats(self):
        if self.monitor:
            ...
            self.stats_label.config(
                text=(
                    f"Alerts: {alert_count} | Active IPs: {active_ips} | Pkts/sec: {pkt_rate} | "
                    f"ARP: {arp_alerts} | ICMP: {icmp_alerts} | TCP: {tcp_alerts} | UDP: {udp_alerts} | "
                    f"Flood-Dropped: {self.flood_dropped_count}"
                )
            )
    ...
    def open_firewall(self):
        if platform.system() == "Windows":
            try:
                # Prevent misuse by ensuring exact command and no shell
                subprocess.Popen(["control.exe", "/name", "Microsoft.WindowsFirewall"], shell=False)
